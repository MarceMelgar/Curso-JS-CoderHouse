...
// 01 - Buscar la película cuyo id es 843633 y mostrar su nombre por pantalla.

/* const resultado = movies.find(el => el.id === 843633)
console.log(resultado.title)

 */

// 02 - Pedir al usuario un nombre de una película y en caso de que se encuentre en el array movies mostrar por pantalla la cantidad de votos que tiene.

/* let pidoNombrePelicula = prompt("Escribe el nombre de una pelicula")

let peliculaEncontrada = movies.find(movie => movie.title === pidoNombrePelicula)
if (peliculaEncontrada) {
  console.log(peliculaEncontrada.vote_count)
}


 */

// 03 - Fitrar las películas más vistas, es decir, las que superan los 1000 puntos de popularidad.

/* let resultados = movies.filter((su) => su.popularity > 1000)
...
// 04 - Mostrar el nombre de cada película concatenado con la fecha en que salió.
/* 
movies.forEach(pelicula =>console.log(pelicula.title, " ", pelicula.release_date)) */

// --

/* let peliculasNombreYFecha = movies.map(movie => movie.title + " " + movie.release_date)
console.log(peliculasNombreYFecha)
 */

// 05 - Ordenar las películas por pularidad de manera ascendente.

/* movies.sort((a,b) => a.popularity - b.popularity)
console.log(movies) */

// 06 - Ordenar las películas por pularidad de manera descendente.

/* movies.sort((a,b) => b.popularity - a.popularity)
console.log(movies) */

// 07 - Ordenar las peliculas por nombre de manera ascendente.
...
// 08 - Ordenar las peliculas por nombre de manera descendente.
...
// 09 - El array contiene mucha información que no uso en mi web por lo tanto quiero guardar en un nuevo array solo el nombre, 
//      id y popularidad de cada una de las películas.

/* let nuevoArray = movies.map( movie => {
  return {id: movie.id, title: movie.title, popularity: movie.popularity}
})
...
// 10 - Tomando el nuevo array del ejercicio 9, agregale a cada película la propiedad views con valor 0 además de las que tenemos actualmente.

/* nuevoArray = movies.map( movie => {
  return {id: movie.id, title: movie.title, popularity: movie.popularity, views: 0}
})

console.log(nuevoArray) */

// 11 - Trabajando sobre el mismo array del ejercicio 9 y 10, me di cuenta que perdí la propuedad adult, ahora deseo crear un nuevo array aagregando 
//      la propiedad "adult" con valor true si el generl es "Horror" o "Thriller" sino false. Las demás propiedades quedan tal cual están, 
//      es decir me quedaría nombre, id, views y adult.

nuevoArray = movies.map( movie => {
  let valorPropiedadAdult = false
  if ((movie.genre === "Horror") || (movie.genre === "Horror")) {
    valorPropiedadAdult = true
  }
  return {id: movie.id, title: movie.title, popularity: movie.popularity, views: movies.views, adult: valorPropiedadAdult }
})
